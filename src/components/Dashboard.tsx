  ...
  return (
    <div className="cq-root">
       ...
       <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '22px 34px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '10px', color: 'rgba(var(--fg),0.35)', letterSpacing: '1.5px', textTransform: 'uppercase' }}>{t('Guided Change Management')}</div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          ...
          {hasSupabase && session && (
            <button type="button" onClick={signOut} style={ghostBtn} title={session.user.email ? tp('Sign out ({email})', { email: session.user.email }) : t('Sign out')}>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '28px 34px' }}>
        {total === 0 ? (
          ...
          <div style={{ maxWidth: '680px', background: 'radial-gradient(560px 240px at 92% -40%, rgba(255,255,255,0.16), transparent 60%), linear-gradient(120deg, #3e6079 0%, #2c4a60 100%)', borderRadius: '16px', padding: narrow ? '24px 22px' : '30px 32px', boxShadow: '0 12px 32px rgba(20,40,55,0.28)' }}>
            <h1 style={{ margin: 0, fontSize: narrow ? '23px' : '27px', fontWeight: 800, color: '#fff', lineHeight: 1.15, letterSpacing: '-0.5px' }}>{t('Lead your change with confidence')}</h1>
            <p style={{ margin: '10px 0 22px', fontSize: '14.5px', color: 'rgba(255,255,255,0.82)', lineHeight: 1.55, maxWidth: '460px' }}>
              {t('Adaptus walks you through rolling out a change from start to finish, no change-management experience required.')}
            </p>
            <button
              type="button"
          ...
          <>
            ...
            {active && nextStage ? (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--accent-text)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '3px' }}>{t('Pick up where you left off')}</div>
                  <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{active.name || t('Untitled project')}</div>
                  <div style={{ fontSize: '13px', color: 'rgba(var(--fg),0.6)', display: 'flex', alignItems: 'center', gap: '6px', marginTop: '2px' }}>
                    {t('Next step:')} <nextStage.icon size={14} color="var(--accent-text)" /> {t(nextStage.label)}
                  </div>
                </div>
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--accent-text)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '3px' }}>{t('All caught up')}</div>
                  <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text)' }}>{t('Start your next change project')}</div>
                </div>
                <span style={{ flexShrink: 0, display: 'inline-flex', alignItems: 'center', gap: '7px', background: 'linear-gradient(135deg, #2dd4bf 0%, #12b3a1 100%)', color: '#06302b', borderRadius: '10px', padding: '11px 18px', fontWeight: 700, fontSize: '14px', boxShadow: '0 4px 14px rgba(18,179,161,0.35)' }}>
                  {t('New project')} <ArrowRight size={17} />
                </span>
              </button>
            )}
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', marginBottom: '14px' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px' }}>
                <span style={{ fontSize: '16px', fontWeight: 700 }}>{t('Your Projects')}</span>
                <span style={{ fontSize: '12px', color: 'rgba(var(--fg),0.62)' }}>
                  {tp(total === 1 ? '{total} project · {completed} complete' : '{total} projects · {completed} complete', { total, completed })}
                </span>
              </div>
              {total > 1 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <select className="cq-select" value={sortKey} onChange={(e) => setSortKey(e.target.value as SortKey)} aria-label={t('Sort projects')} style={{ width: 'auto', fontSize: '13px' }}>
            ...
            {visible.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '48px 40px', color: 'rgba(var(--fg),0.5)', fontSize: '14px', background: 'rgba(var(--fg),0.02)', border: '1px dashed rgba(var(--fg),0.1)', borderRadius: '16px' }}>
                {tp('No projects match “{query}”.', { query })}
              </div>
            ) : (
      ...
      {deleted && (
        ...
        >
          <span style={{ fontSize: '13px', color: 'var(--text)' }}>{tp('Deleted “{name}”', { name: deleted.name || t('Untitled project') })}</span>
          <button
            type="button"
            onClick={undoDelete}
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px', marginBottom: '12px' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '7px', marginBottom: '4px', minHeight: '37px' }}>
            <div style={{ flex: 1, minWidth: 0, fontSize: '14px', fontWeight: 700, lineHeight: 1.3, color: name ? 'var(--text)' : 'rgba(var(--fg),0.45)', fontStyle: name ? 'normal' : 'italic', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{name || t('Untitled project')}</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '11px', color: 'rgba(var(--fg),0.55)', minWidth: 0 }}>
            ...
            {shared && <span style={{ flexShrink: 0, color: 'var(--accent-text)', border: '1px solid rgba(91,134,163,0.3)', borderRadius: '6px', padding: '1px 6px', fontWeight: 600 }}>{tp('Shared · {role}', { role: t(role === 'editor' ? 'Editor' : 'Viewer') })}</span>}
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
          {/* Revealed on hover for mouse users; always shown on touch (no hover). */}
          {canEdit && (
            <button type="button" style={{ ...cardIconBtn, opacity: actionsShown ? 1 : 0, pointerEvents: actionsShown ? 'auto' : 'none', transition: 'opacity 0.15s' }} title={t('Edit project details')} aria-label={t('Edit project details')} onClick={stop(onEdit)}><Pencil size={13} /></button>
          )}
          {canDelete && (
            <button type="button" style={{ ...cardIconBtn, color: '#fca5a5', opacity: actionsShown ? 1 : 0, pointerEvents: actionsShown ? 'auto' : 'none', transition: 'opacity 0.15s' }} title={t('Delete project')} aria-label={t('Delete project')} onClick={stop(onDelete)}><Trash2 size={13} /></button>
          )}
        </div>
      </div>
      <div style={{ marginBottom: '12px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
          <span style={{ fontSize: '11px', color: 'rgba(var(--fg),0.55)' }}>{t('Progress')}</span>
          {complete ? (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '11px', fontWeight: 700, color: '#86efac' }}><Check size={12} strokeWidth={3} /> {t('Complete')}</span>
          ) : (
      ...
      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginTop: 'auto' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: 'rgba(var(--fg),0.04)', border: '1px solid rgba(var(--fg),0.07)', borderRadius: '8px', padding: '4px 9px', fontSize: '11px', color: 'rgba(var(--fg),0.62)' }}><StageIcon size={12} /> {t(stageTag)}</div>
        {avg !== null && (
          <div style={{ background: `rgba(${riskRgb},0.1)`, border: `1px solid rgba(${riskRgb},0.25)`, borderRadius: '8px', padding: '4px 9px', fontSize: '11px', color: riskColor(avg) }}>
            {tp('Risk: {level}', { level: t(riskLabel(avg)) })}
          </div>
        )}
        <div style={{ background: 'rgba(var(--fg),0.04)', border: '1px solid rgba(var(--fg),0.07)', borderRadius: '8px', padding: '4px 9px', fontSize: '11px', color: 'rgba(var(--fg),0.55)' }}>
          {tp('{done} of {total} essential steps', { done: coreDone, total: ESSENTIAL_COUNT })}
        </div>
      </div>
    </div>
  )
