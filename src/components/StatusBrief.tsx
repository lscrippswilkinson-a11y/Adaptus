...
export function StatusBrief({ project, publicView = false, premium = false }: { project: Project; publicView?: boolean; premium?: boolean }) {
  const sd = project.stageData
  const prep = preparedness(project)
  const avg = avgRisk(sd.risk.items)
  // The user's own logo + colour (Premium), or the Adaptus default. On the public
  // page `premium` is the OWNER's plan, since the viewer has no account at all.
  ...
  // White-label: when on (Premium only), the brief carries no "Adaptus" mark and no CTA.
