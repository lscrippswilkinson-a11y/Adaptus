...
export function AdoptionStage() {
  ...
  data.metrics.forEach((m, i) => {
    ...
    steps.push({
      ...
      node: (
        <div>
          ...
          <TextInput value={m.name} onCommit={(v) => setMetric(m.id, { name: v })} placeholder="Example: Timekeepers entering their own time" />
    ...
    steps.push({
      ...
      node: (
        <div>
          ...
          <TextInput value={m.target} onCommit={(v) => setMetric(m.id, { target: v })} placeholder="Example: 90" />
        </div>
      ),
    })
    ...
    steps.push({
      ...
      node: (
        <div>
          ...
          <TextInput value={m.current} onCommit={(v) => setMetric(m.id, { current: v })} placeholder="Example: 55" />
          {hasProgress && (
    ...
    steps.push({
      id: `${m.id}-checkBy`,
      title: `${what}: next check`,
      isFilled: !!m.name.trim(),
      summary: m.checkBy ? `Next check ${longDate(m.checkBy)}` : undefined,
      emptyLabel: 'No check-in date set',
      node: (
        <div>
