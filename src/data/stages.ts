...
export const STAGES: Stage[] = [
  ...
  { id: 'stakeholders', label: 'Get key people on side', icon: Handshake, tag: 'Key people', xp: 120, phase: 'planning', tier: 'premium' },
  { id: 'risk', label: 'What could go wrong', icon: Zap, tag: 'Risks', xp: 150, phase: 'planning', tier: 'premium' },
  { id: 'resistance', label: 'Handling pushback', icon: Shield, tag: 'Pushback', xp: 130, phase: 'planning', tier: 'premium' },
  ...
  { id: 'testing', label: 'Test Before Launch', icon: FlaskConical, tag: 'Testing', xp: 110, phase: 'planning', tier: 'premium' },
  { id: 'dependencies', label: 'What you’re waiting on', icon: Link2, tag: 'Waiting on', xp: 110, phase: 'planning', tier: 'premium' },
  ...
  { id: 'sustainment', label: 'Make it stick', icon: RefreshCw, tag: 'Keep it going', xp: 150, phase: 'postlaunch', tier: 'premium' },
  { id: 'executive', label: 'How the launch went', icon: BarChart3, tag: 'Report', xp: 200, phase: 'postlaunch', tier: 'essential' },
]

/** Section ids that make up the core (essential) path — free, always. */
...
/** Section ids behind the Premium plan. Progress is measured on essentials
 *  only (see `pct`), so locking these never changes anyone's percentage. */
export const PREMIUM_IDS = new Set(STAGES.filter((s) => s.tier === 'premium').map((s) => s.id))
export const PREMIUM_COUNT = PREMIUM_IDS.size
