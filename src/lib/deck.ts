...
function buildStatusSlide(pptx: any, project: Project, deck: Deck) {
  ...
  const statusWord = pct >= 80 ? t('On track') : pct >= 50 ? t('At risk') : t('Needs attention')
  const goLive = sd.milestones.goLiveDate
    ...
    : project.targetDate || '-'
  const longDate = new Date().toLocaleDateString(htmlLang(getLang()), { month: 'long', day: 'numeric', year: 'numeric' })
  ...
  slide.addText(project.name || t('Change project'), { x: 0.5, y: 0.4, w: 9.4, h: 0.6, fontSize: 26, bold: true, color: deck.BAND_FG, valign: 'middle' })
  slide.addText(`${project.type ? t(project.type) : t('Change project')}   ·   ${t('Status Brief')}   ·   ${longDate}`, { x: 0.5, y: 1.0, w: 9.4, h: 0.3, fontSize: 11, color: LIGHT })
  slide.addShape('roundRect', { x: 10.5, y: 0.48, w: 2.33, h: 0.52, rectRadius: 0.08, fill: { color: statusColor } })
  slide.addText(`${statusWord} · ${tp('{pct}% ready', { pct })}`, { x: 10.5, y: 0.48, w: 2.33, h: 0.52, fontSize: 11, bold: true, color: '11141F', align: 'center', valign: 'middle' })

  // Metric tiles
  const tiles = [
    { v: `${pct}%`, l: t('Launch ready'), c: statusColor },
    { v: goLive, l: t('Go-live'), c: 'FFFFFF' },
    { v: prep.total ? `${prep.done}/${prep.total}` : '-', l: t('Steps complete'), c: 'FFFFFF' },
  ]
  ...
  tiles.forEach((tile, i) => {
    ...
    slide.addText(tile.v, { x: x + 0.22, y: tileY + 0.12, w: tileW - 0.44, h: 0.6, fontSize: tile.v.length > 8 ? 20 : 28, bold: true, color: tile.c, valign: 'middle' })
    slide.addText(tile.l.toUpperCase(), { x: x + 0.22, y: tileY + 0.74, w: tileW - 0.44, h: 0.26, fontSize: 10, color: MUTED, charSpacing: 1 })
  })

  // Left column: top risks
  const colY = 3.2
  slide.addText(t('TOP RISKS TO WATCH'), { x: 0.5, y: colY, w: 6, h: 0.3, fontSize: 12, bold: true, color: LIGHT, charSpacing: 1 })
  let ry = colY + 0.5
  ...
  if (sd.sponsor.noSponsor) addRisk(t('No senior backer identified'), t('Critical'), 'EF4444')
  if (topRisks.length) topRisks.forEach((r) => addRisk(r.description, t(riskLabel(r.score)), hx(riskColor(r.score))))
    ...
    if (avg !== null) addRisk(tp('Overall risk going in: {level} ({score}/10)', { level: t(riskLabel(avg)), score: avg }), null, hx(riskColor(avg)))
    else slide.addText(t('No risks logged yet.'), { x: 0.85, y: ry, w: 5.6, h: 0.3, fontSize: 12, italic: true, color: MUTED })
  }

  // Right column: coalition + the ask
  const rx = 7.0
  slide.addText(t('WHO’S ON BOARD?'), { x: rx, y: colY, w: 5.83, h: 0.3, fontSize: 12, bold: true, color: LIGHT, charSpacing: 1 })
  const sponsorLine = sd.sponsor.noSponsor
    ? t('⚠ No senior backer — flagged as a risk')
    : sd.sponsor.name
      ? tp('Backer: {name}', { name: `${sd.sponsor.name}${sd.sponsor.role ? ` (${sd.sponsor.role})` : ''}` })
      : t('No backer named yet')
  ...
  if (named.length) {
    const parts = [tp('{count} on board', { count: advocates })]
    if (resistant) parts.push(tp('{count} to win over', { count: resistant }))
    parts.push(tp('{count} listed', { count: named.length }))
  ...
  if (branded) slide.addText(t('Made with Adaptus'), { x: 0.5, y: 7.08, w: 6, h: 0.3, fontSize: 10, color: '6B7A88' })
}
...
function buildTasksSlides(pptx: any, project: Project, deck: Deck) {
  ...
  if (prep.total === 0 || openTasks.length === 0) {
    ...
    const slide = addContentSlide(pptx, t('What’s left before launch'), deck)
    slide.addText(
      prep.total === 0 ? t('Launch tasks haven’t been mapped yet.') : tp('✓ All {total} tasks complete — ready to launch.', { total: prep.total }),
  ...
  let slide = addContentSlide(pptx, t('What’s left before launch'), deck)
  let y = BODY_TOP
  const nextPage = () => { slide = addContentSlide(pptx, t('What’s left before launch (cont.)'), deck); y = BODY_TOP }
  ...
  for (const { group, items } of openByGroup) {
    ...
    slide.addText(`${groupLabel(group).toUpperCase()}   ·   ${tp('{count} left', { count: items.length })}`, { x: 0.5, y, w: 12.3, h: 0.3, fontSize: 12, bold: true, color: deck.LIGHT, charSpacing: 1 })
    y += 0.44

    for (const task of items) {
      const hasSub = !!(task.owner || task.due)
      const rowH = 0.3 * estLines(task.label, 108) + (hasSub ? 0.26 : 0.14)
      if (y + rowH > BODY_BOTTOM) {
        nextPage()
        slide.addText(`${groupLabel(group).toUpperCase()} ${t('(CONT.)')}`, { x: 0.5, y, w: 12.3, h: 0.3, fontSize: 12, bold: true, color: deck.LIGHT, charSpacing: 1 })
        y += 0.44
      }
      slide.addShape('roundRect', { x: 0.55, y: y + 0.03, w: 0.17, h: 0.17, rectRadius: 0.03, fill: { color: deck.BG }, line: { color: '6E7C8A', width: 1 } })
      slide.addText(task.label, { x: 0.9, y: y - 0.03, w: 11.85, h: 0.3 * estLines(task.label, 108), fontSize: 12.5, color: deck.TEXT, valign: 'top' })
      if (hasSub) {
        const sub = [task.owner ? tp('Owner: {owner}', { owner: task.owner }) : '', task.due ? tp('Due {date}', { date: shortDate(task.due) }) : ''].filter(Boolean).join('     ·     ')
        slide.addText(sub, { x: 0.9, y: y + 0.3 * estLines(task.label, 108) - 0.06, w: 11.85, h: 0.24, fontSize: 10, color: deck.SUB })
      }
      y += rowH
    }
    y += 0.16
...
function buildTimelineSlide(pptx: any, project: Project, deck: Deck) {
  ...
  let slide = addContentSlide(pptx, t('Launch timeline'), deck)
  let y = BODY_TOP
  for (const entry of timeline) {
    const suffix = [entry.owner, entry.postLaunch ? t('after launch') : ''].filter(Boolean).join('   ·   ')
    const label = (entry.milestone ? '🚀 ' : '') + entry.label + (suffix ? `   ·   ${suffix}` : '')
    const rowH = 0.3 * estLines(label, 104) + 0.2
    if (y + rowH > BODY_BOTTOM) { slide = addContentSlide(pptx, t('Launch timeline (cont.)'), deck); y = BODY_TOP }
    slide.addText(shortDate(entry.date), { x: 0.5, y, w: 1.1, h: 0.3, fontSize: 12, bold: true, color: deck.LIGHT, valign: 'top' })
    slide.addText(label, {
      x: 1.7, y, w: 11.1, h: 0.3 * estLines(label, 104),
      fontSize: 12.5, bold: entry.milestone, strike: entry.done, color: entry.done ? deck.SUB : deck.TEXT, valign: 'top',
    })
    y += rowH
  }
}
...
function buildAdoptionSlide(pptx: any, project: Project, deck: Deck) {
  ...
  let slide = addContentSlide(pptx, t('Real use'), deck)
  let y = BODY_TOP
  for (const m of metrics) {
    if (y + 0.95 > BODY_BOTTOM) { slide = addContentSlide(pptx, t('Real use (cont.)'), deck); y = BODY_TOP }
    ...
    if (m.current) slide.addText(`${m.current}${t(m.unit)} / ${m.target}${t(m.unit)}`, { x: 9.4, y, w: 3.4, h: 0.3, fontSize: 13, bold: true, color: deck.LIGHT, align: 'right', valign: 'top' })
    y += 0.4
    ...
    y += 0.26
    if (has) {
      slide.addText(status.label, { x: 0.5, y, w: 6, h: 0.25, fontSize: 10, bold: true, color: status.c })
      y += 0.4
    } else {
      y += 0.24
    }
